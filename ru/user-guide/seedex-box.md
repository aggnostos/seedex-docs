...
Сервис VPN держит туннели AmneziaWG. Каждый конфиг — отдельный туннель. Роутер пробует все и ведёт трафик через самый быстрый живой туннель. Трафик идёт через туннель только пока работает сервис Router: сам по себе VPN поднимает туннель, но ничего через него не отправляет.
...
Сервис proxy держит туннель sing-box. Каждый конфиг отдаёт свои outbound-ы одному экземпляру sing-box, который выбирает лучший outbound по URL-тесту. Роутер видит результат как один туннель рядом с туннелями VPN. Как и с VPN, трафик идёт через proxy только пока работает сервис Router.
