...
* `sdx import <path|link>` импортирует файл конфига, все конфиги из каталога или ссылку на прокси. Файл `.conf` AmneziaWG идёт в `vpn`, файл `.json` sing-box — в `proxy`, файл `.json` с правилами — в `router`. Имя файла становится именем записи. Занятое имя отклоняется.
  * Ссылки: `vless://`, `trojan://`, `ss://`, `vmess://`, `hysteria2://` (`hy2://`), `tuic://` и `anytls://` — в том виде, в каком ими делятся клиенты и панели. Ссылка становится конфигом `proxy` с именем из `#tag`. Текстовый файл со ссылкой на строку импортирует все ссылки.
...
Сервис VPN держит туннели AmneziaWG. Каждый конфиг — отдельный туннель. Роутер пробует все и ведёт трафик через самый быстрый живой туннель. Трафик идёт через туннель только через сервис Router, поэтому запуск VPN заодно запускает сервисы Router и DNS, если они не работают.
...
Сервис proxy держит туннель sing-box. Каждый конфиг отдаёт свои outbound-ы одному экземпляру sing-box, который выбирает лучший outbound по URL-тесту. Роутер видит результат как один туннель рядом с туннелями VPN. Как и с VPN, запуск proxy заодно запускает сервисы Router и DNS, если они не работают.
...
Матчеры по назначению. Матчер принимает одно значение или несколько через запятую, его можно повторять:
...
* `sdx router update <#|name> ...` меняет правило. Команда принимает `type=`, `name=` и опции списка и редактирует списки матчеров `domain`, `ip`, `client_mac` и `client_ip`:
  * `domain=` заменяет список. `domain=` очищает его.
