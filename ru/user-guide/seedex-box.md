...
`iface=<конфиг>` закрепляет правило `overlay` за одним туннелем по имени его конфига VPN или Proxy, вместо самого быстрого. Пока этот туннель лежит, его трафик идёт по маршруту overlay по умолчанию, kill switch при этом действует. Закреплённые правила проверяются раньше остальных:
