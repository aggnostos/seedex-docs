...
* `sdx import <path|link|url>` импортирует файл конфига, все конфиги из каталога, ссылку на прокси, адрес подписки. Файл `.conf` WireGuard (WG) или AmneziaWG (AWG) идёт в `vpn`, файл `.json` sing-box — в `proxy`, файл `.json` с правилами — в `router`. Имя файла становится именем записи. Занятое имя отклоняется.
