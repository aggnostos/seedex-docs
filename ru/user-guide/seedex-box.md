...
* `sdx import <path|link>` импортирует файл конфига, все конфиги из каталога или ссылку на прокси. Файл `.conf` WireGuard (WG) или AmneziaWG (AWG) идёт в `vpn`, файл `.json` sing-box — в `proxy`, файл `.json` с правилами — в `router`. Имя файла становится именем записи. Занятое имя отклоняется.
...
Сервис VPN держит туннели WG и AWG. Каждый конфиг — отдельный туннель: `.conf` с параметрами обфускации AWG поднимается на интерфейсе `awg`, обычный `.conf` WG — на интерфейсе `wg`. Роутер пробует все и ведёт трафик через самый быстрый живой туннель. Трафик идёт через туннель только через сервис Router, поэтому запуск VPN заодно запускает сервисы Router и DNS, если они не работают.
