...
1. На роутере запустите установщик от root:
   ...
   Установщик подключает фид пакетов Seedex, ставит `seedex-box` и `luci-app-seedex` для LuCI и запускает DNS. Чтобы обойтись без LuCI, запустите установщик с `| sh -s -- --no-luci`.
...
2. Вставьте строку, которую напечатал сервер:
   ...
   Команда открывает меню с конфигами, которые предлагает сервер. Перемещайтесь стрелками, отмечайте конфиг пробелом и подтвердите Enter. Роутер импортирует выбранные конфиги, поднимает туннели и начинает маршрутизацию.
