...
* **Router**: политики маршрутизации решают, что идёт через туннель, что напрямую к провайдеру и что блокируется — по домену, списку, устройству и подсети. Kill switch блокирует трафик, когда ни один туннель не поднят.
* **DNS**: приватный резолвер для всей сети с шифрованным upstream и перехватом устройств, которые резолвят сами.
* **Link**: связь с [seedex-agent](https://github.com/aggnostos/seedex-agent). Роутер забирает конфиги с сервера и управляет сервером удалённо.
